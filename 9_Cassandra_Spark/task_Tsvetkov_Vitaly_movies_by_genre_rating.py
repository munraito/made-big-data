...
ratings = ratings.select(col('movieId'), col('rating')).withColumnRenamed('movieId', 'movieid')
ratings = ratings.groupBy('movieid').agg(avg('rating').alias('rating'))
df = df.join(ratings, on='movieid', how='inner')
df = df.withColumn('genre', explode('genres')).drop('genres')

df.write.format("org.apache.spark.sql.cassandra") \
        .options(table="movies_by_genre_rating", keyspace=KEYSPACE_NAME) \
        .mode("append").save()
